...
export namespace Format {
	...
	export function formatHttp(site: string | undefined) {
		...
		return trimmed ? (trimmed.startsWith('http') ? trimmed : `https://${trimmed}`) : undefined;
	}
	...
	export function capitalize(value: string | undefined) {
		...
		return /^[A-Z]$/.test(stringValue[0]) ? stringValue : _.capitalize(stringValue);
	}
	...
	export function dateDiffDays(to: string | Date | number, from?: string | Date | number) {
		return moment(from).diff(moment(to), 'days') + ' days';
	}

	export function dateDiffDdays(to: string | Date | number, from?: string | Date | number) {
	...
	export function timeForDataTable(target: DateType) {
		return [Format.dateDH(target), `${Format.time(target)} (${Format.dateAgo(target)})`];
	}
	export function duration(arg: { target: DateType; current?: DateType; unit?: unitOfTime.Base }) {
	...
	export function dateFull(date?: DateType, format: string = 'MMM Do YYYY, HH:mm') {
		if (_.isNil(date)) {
			return '-';
		}
	...
	export function floatWithDecimalNoCutoff(value: number | string, decimal?: number) {
	...
	export function updateAt(date: DateType) {
		return date ? `Updated at ${moment(date).format('MMMM Do, HH:mm:ss')}` : '-';
	}

	export function updateAtUTC(date: DateType) {
		return date ? `Updated at ${moment(date).utc(false).format('YYYY-MM-DD HH:mm:ss z')}` : '-';
	}
	...
	export function integer(value: number | string | undefined) {
		return isNaN(Number(value)) ? '-' : formatNumberWithCommas(value as number | string);
	...
	export function percent1(
		...
		if (value === undefined || isNaN(Number(value)) || Number(value) === Infinity) {
			return '-';
		} else {
			let output = '0.0%';
	...
	export function price(value: number | string | undefined, cutoff: number | null = null) {
	...
	export function price1(value: number | string | undefined, cutoff: number | null = null) {
		if (value === undefined) {
			return '-';
		}
	...
	export function price2(value: number | string | undefined, cutoff: number | null = null) {
		if (value === undefined) {
			return '-';
		}
	...
	export function price3(value: number | string | undefined, cutoff: number | null = null) {
		if (value === undefined) {
			return '-';
		}
		const _cutoff = cutoff !== null ? (cutoff < 1e-5 ? 1e-5 : cutoff) : 1e-5;
	...
	export function price4(value: number | string | undefined, cutoff: number | null = null) {
		if (value === undefined) {
			return '-';
		}
		const _cutoff = cutoff !== null ? (cutoff < 1e-5 ? 1e-5 : cutoff) : 1e-5;
	...
	export function formatAutoPercent(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormat(Number(value) * 100) + '%', Number(value))
			: Number(value) < cutoff
				? '< ' + Number(value) * 100 + '%'
				: Format.withFormatter(getMeanfulFormat(Number(value) * 100) + '%', Number(value));
	}
	...
	export function formatAutoFloatTight(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormatTight(Number(value)), Number(value))
			: Number(value) < cutoff
				? '< ' + cutoff
				: Format.withFormatter(getMeanfulFormatTight(Number(value)), Number(value));
	}
	...
	export function formatAutoFloatTight2x(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormatTight2x(Number(value)), Number(value))
			: Number(value) < cutoff
				? '< ' + cutoff
				: Format.withFormatter(getMeanfulFormatTight2x(Number(value)), Number(value));
	}
	...
	export function formatAutoFloatLoose(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormatLoose(Number(value)), Number(value))
			: Number(value) < cutoff
				? '< ' + cutoff
				: Format.withFormatter(getMeanfulFormatLoose(Number(value)), Number(value));
	}
	export function formatAutoPercentLoose(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormatLoose(Number(value) * 100) + '%', Number(value))
			: Number(value) < cutoff
				? '< ' + cutoff
				: Format.withFormatter(getMeanfulFormatLoose(Number(value) * 100) + '%', Number(value));
	}
	export function formatAutoPercentTight(
		...
		return cutoff === null
			? Format.withFormatter(getMeanfulFormatTight(Number(value) * 100) + '%', Number(value))
			: Number(value) < cutoff
				? '< ' + cutoff
				: Format.withFormatter(getMeanfulFormatTight(Number(value) * 100) + '%', Number(value));
	}
	...
	export function graphTrendsDate(date?: string | Date | number, format: string = 'MM/DD HH:mm') {
		return moment(date).format(format);
	}

	export function graphTrendsDay(date?: string | Date | number, format: string = 'MM/DD') {
		return moment(date).format(format);
	}
	...
	export function dhNoFloat(value: number | string) {
		return Number(value) < 1000 ? numeral(value).format('0,0') : numeral(value).format('0,0.00a');
	}
	...
	export function floatSplit(
		...
		return {
			...
			decimal: arr[1] || suffix ? `${arr[1] || 0}${suffix ? ' ' + suffix : ''}` : '',
			origin: value || 0,
	...
	export function floatHtml(
		...
		const { decimal = 6, decimalFontSize = 0.72, formatter, suffix, opacity = 0.8 } = option || {};
		if (value === undefined) {
			return '-';
		}
		const origin = formatter ? formatter(value) : Number(value).toFixed(decimal);
		const arr = origin.split('.');
	...
	export function plural(prefix: string, items: any[], isStartE: boolean = false) {
	...
	export function httpURL(site: string | undefined) {
		return site ? (site.startsWith('http') ? site : `https://${site}`) : undefined;
	}
	export function dateGraph(date?: string | Date | number, format: string = 'MM/DD HH:mm') {
		return moment(date).format(format);
	}
	...
	export function removeGithubTextIcon(value: string | undefined, defaultText?: string) {
	...
	export function diff(seconds: number | string) {
		...
		Number(seconds) > 60 && Number(seconds) < 3600 && (result = `${Number(seconds) / 60} mins`);
		Number(seconds) === 3600 && (result = `${Number(seconds) / 3600} hour`);
		Number(seconds) > 3600 &&
			Number(seconds) < 3600 * 24 &&
			(result = `${Number(seconds) / 3600} hours`);
		Number(seconds) === 3600 * 24 && (result = `${Number(seconds) / (3600 * 24)} day`);
		Number(seconds) > 3600 * 24 && (result = `${Number(seconds) / (3600 * 24)} days`);
