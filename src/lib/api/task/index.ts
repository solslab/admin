...
export namespace __Task {
  ...
  export async function createPosition(
    args: __Model.CreatePositionRequest,
  ): Promise<any> {
    const url = `/company/${args.companyId}/position`;
    const body = {
  ...
  export async function updatePosition(
    args: __Model.UpdatePositionRequest,
  ): Promise<any> {
    const url = `/position/${args.positionId}`;
    const body = {
      position_name: args.position_name,
      support_languages: args.support_languages,
      test_time: args.test_time ?? null,
  ...
  // 직무를 삭제하는 함수
  export async function deletePosition(args: {
    positionId: string;
    token: string;
  }): Promise<any> {
    const url = `/position/${args.positionId}`;
