...
export function isUnderScreenAndContain(standard: ScreenType, current?: ScreenType) {
	const _currentScreen = current || currentScreen;
	return screenSizeOrder[standard] >= screenSizeOrder[_currentScreen];
}
...
export function isOverScreenAndContain(standard: ScreenType, current?: ScreenType) {
	const _currentScreen = current || currentScreen;
	return screenSizeOrder[standard] <= screenSizeOrder[_currentScreen];
}

export function isScreenContains(current: ScreenType, ...standard: ScreenType[]) {
	const _currentScreen = current;

	return standard.some((s) => screenSizeOrder[s] === screenSizeOrder[_currentScreen]);
}
...
export namespace Screen {
	...
	export function responsive<T>(
		...
		const { xs = null, sm = null, md = null, lg = null, xl = null, xxl = null, fxl = null } = arg;
		switch (screen) {
			case 'xs':
				return (
					xs !== null
